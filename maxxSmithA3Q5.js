let darken;
let pizzaX;
let pizzaY;
let speedY;
let pizzaIsFalling;
let pizzaTime;
let gameOver;
let pizzasEaten;
let showClickToStart;
let startTimer;

function setup() {
  createCanvas(400, 400);
  angleMode(DEGREES);
  textAlign(CENTER);

  pizzaX = random(0, 380);
  pizzaY = 0;
  darken = 15;
  speedY = random(1, 5);
  pizzaTime = 180;
  gameLost = false;
  pizzasEaten = 0;
  showClickToStart = true;
  startTimer = false;
  setInterval(timer, 1000);
}

function draw() {
  background(132, 176, 209);

  //Draws the Room
  drawScenery();
  //Creates Pizza Man
  drawPizzaMan(mouseX - 80, 250, 0.5);
  //Shows popups
  textPopups();
  //Creates game function
  gameFunctionality();
  //Draws timer and pizza counter
  displayCounters();
}

function drawPizzaMan(x, y, size) {
  //Colors
  let skinColor = color(255, 227, 201);
  let darkSkin = color(255 - darken, 227 - darken, 201 - darken);
  let uniform = color(250, 82, 82);
  let darkUniform = color(250 - darken, 82 - darken, 82 - darken);
  let pants = color(38, 38, 46);

  //Places pizza man according to call function
  push();
  translate(x, y);
  scale(size);
  push();

  //Orients and moves pizza man into top left corner
  translate(-35, 275);
  rotate(-90);
  translate(100, 0);

  noStroke();

  //Neck
  fill(darkSkin);
  rect(130, 240, 25, 60);
  arc(130, 300, 50, 50, 0, 90);
  rect(70, 300, 60, 25);

  //Arms
  fill(darkSkin);
  rect(-40, 220, 80, 25, 10);
  push();
  translate(0, 165);
  rect(-40, 220, 80, 25, 10);
  pop();

  //Uniform
  fill(uniform);
  rect(3, 250, 70, 130, 10);
  fill(darkUniform);
  arc(70, 315, 50, 80, 270, 315);
  arc(70, 315, 50, 80, 45, 90);
  beginShape();
  vertex(63, 250);
  vertex(23, 250);
  vertex(23, 210);
  vertex(63, 225);
  endShape(CLOSE);
  push();
  translate(0, 170);
  beginShape();
  vertex(63, 235);
  vertex(23, 250);
  vertex(23, 210);
  vertex(63, 210);
  endShape(CLOSE);
  pop();

  //Pants
  fill(pants);
  rect(-50, 250, 60, 130, 10, 0, 0, 10);

  //Legs
  fill(pants);
  beginShape();
  vertex(-50, 265);
  vertex(-80, 280);
  vertex(-80, 295);
  vertex(-50, 310);
  endShape(CLOSE);
  push();
  beginShape();
  translate(0, 50);
  vertex(-50, 265);
  vertex(-80, 280);
  vertex(-80, 295);
  vertex(-50, 310);
  endShape(CLOSE);
  pop();

  //Feet
  fill("saddlebrown");
  rect(-90, 260, 20, 50, 10);
  rect(-90, 320, 20, 50, 10);

  //Head
  fill(skinColor);
  rect(100, 100, 100, 100);
  arc(150, 200, 100, 100, 0, 180);
  arc(150, 100, 100, 100, 180, 0);
  
    //Open Mouth. Catch area is 190 +- 50
    fill("black");
    ellipse(190, 180, 50, 60);
    fill("red");
    ellipse(190, 195, 40, 20);
    //Top Teeth
    fill("white");
    rect(170, 154, 10, 15, 4);
    rect(180, 150, 10, 15, 4);
    rect(190, 150, 10, 15, 4);
    rect(200, 154, 10, 15, 4);
    //Bottom Teeth
    rect(170, 192, 10, 15, 4);
    rect(180, 196, 10, 15, 4);
    rect(190, 196, 10, 15, 4);
    rect(200, 192, 10, 15, 4);
    //Lips
    push();
    noFill();
    stroke(skinColor);
    strokeWeight(5);
    ellipse(190, 180, 53, 63);
    pop();

  //Test for collision of pizza and pizza mans mouth
  if (pizzaY >= 220 && pizzaY <= 250 && pizzaX > x + 40 && pizzaX < x + 75) {
    push();
    // //Closed Mouth
    // noFill();
    // strokeWeight(20);
    // stroke(skinColor);
    // arc(190, 180, 50, 50, 0, 180);
    // stroke("black");
    // strokeWeight(6);
    // arc(190, 180, 50, 50, 0, 180);

    //Resets Pizza
    pizzaY = 0;
    pizzaIsFalling = false;
    pizzaX = random(0, 380);
    pizzaIsFalling = true;

    //Counts Pizzas Eaten
    pizzasEaten++;
    pop();
  } else {
    // // Open Mouth. Catch area is 190 +- 50
    // fill("black");
    // ellipse(190, 180, 50, 60);
    // fill("red");
    // ellipse(190, 195, 40, 20);
    // //Top Teeth
    // fill("white");
    // rect(170, 154, 10, 15, 4);
    // rect(180, 150, 10, 15, 4);
    // rect(190, 150, 10, 15, 4);
    // rect(200, 154, 10, 15, 4);
    // //Bottom Teeth
    // rect(170, 192, 10, 15, 4);
    // rect(180, 196, 10, 15, 4);
    // rect(190, 196, 10, 15, 4);
    // rect(200, 192, 10, 15, 4);
    // //Lips
    // push();
    // noFill();
    // stroke(skinColor);
    // strokeWeight(5);
    // ellipse(190, 180, 53, 63);
    // pop();
  }

  //Ear
  fill(darkSkin);
  arc(105, 170, 50, 50, 90, 270);

  //Hair
  fill(214, 196, 122);
  arc(100, 100, 100, 100, 0, 100);

  //Hat
  fill(darkUniform);
  arc(150, 90, 110, 110, 180, 0);
  fill(uniform);
  rect(95, 75, 180, 30, 4);

  //Eyes
  fill("white");
  ellipse(200, 110, 45, 45);
  ellipse(145, 110, 45, 45);

  //Iris
  fill("deepskyblue");

  //Makes angry face because he lost
  if (gameOver) {
    fill("red");
    push();
    fill(214, 196, 122);
    rotate(10);
    rect(130, 50, 50, 10);
    rotate(-20);
    rect(160, 110, 50, 10);
    pop();
  }
  ellipse(208, 110, 25, 25);
  ellipse(153, 110, 25, 25);

  //Pupils
  fill("black");
  ellipse(210, 110, 15, 15);
  ellipse(155, 110, 15, 15);

  //Nose
  fill(darkSkin);
  rect(170, 110, 20, 40, 0, 0, 10, 10);
  pop();
  pop();
}
function drawScenery() {
  push();
  noStroke();
  //Colors
  let floor = color(227, 200, 127);
  let wall = color(212, 90, 66);
  let darkWall = color(212 - darken, 90 - darken, 66 - darken);

  //Floor
  fill(floor);
  beginShape();
  vertex(0, 400);
  vertex(0, 350);
  vertex(100, 210);
  vertex(300, 210);
  vertex(400, 350);
  vertex(400, 400);
  endShape(CLOSE);
  fill(wall);

  //Left Wall
  beginShape();
  vertex(0, 350);
  vertex(0, 0);
  vertex(100, 0);
  vertex(100, 210);
  endShape();

  //Right Wall
  beginShape();
  vertex(400, 350);
  vertex(400, 0);
  vertex(300, 0);
  vertex(300, 210);
  endShape();

  //Back Wall
  fill(darkWall);
  rect(100, 0, 200, 210);

  //Counter
  push();
  fill("darkgray");
  translate(10, 0);
  beginShape();
  vertex(100, 210);
  vertex(0, 350);
  vertex(0, 270);
  vertex(100, 130);
  endShape();
  fill("dimgray");
  rect(-80, 270, 80, 80);
  pop();
  push();
  fill("silver");
  rect(-66, 265, 80, 10);
  fill("lightgray");
  beginShape();
  vertex(12, 265);
  vertex(12, 275);
  vertex(112, 135);
  vertex(112, 125);
  endShape(CLOSE);
  pop();
  push();
  fill("gainsboro");
  beginShape();
  vertex(12, 265);
  vertex(112, 125);
  vertex(90, 125);
  vertex(-12, 265);
  endShape(CLOSE);
  pop();

  //Door
  fill("burlywood");
  rect(140, 50, 120, 160);
  fill("black");
  rect(150, 60, 100, 150);

  //Order Window
  push();
  translate(20, -50);
  push();
  fill("grey");
  translate(-5, 0);
  beginShape();
  vertex(300, 210);
  vertex(300, 90);
  vertex(400, 220);
  vertex(400, 350);
  endShape(CLOSE);
  pop();
  fill("lightskyblue");
  beginShape();
  vertex(300, 210);
  vertex(300, 100);
  vertex(400, 240);
  vertex(400, 350);
  endShape(CLOSE);
  pop();
  pop();
}
function drawPizza(x, y, size) {
  push();
  //Translates pizza according to called function
  translate(x, y);
  scale(size);
  push();
  translate(-5, -5);
  noStroke();
  //Cheese
  fill("yellow");
  triangle(10, 10, 40, 10, 25, 40);

  //Peperonis
  fill("red");
  ellipse(20, 20, 6);
  ellipse(30, 15, 6);
  ellipse(25, 30, 6);

  //Crust
  fill(186, 161, 128);
  rect(7, 6, 35, 5, 10);
  pop();
  pop();
}
function textPopups() {
  //Pops up text for game loss
  if (gameOver) {
    push();
    textSize(50);
    fill("red");
    stroke("white");
    strokeWeight(5);
    text("Game Over", width / 2, height / 2);
    pop();

    startTimer = false;
  }

  //Shows Click To Start Instruction
  if (showClickToStart == true) {
    push();
    textSize(50);
    fill("green");
    stroke("white");
    strokeWeight(5);
    text("Click To Start", width / 2, height / 2);
    pop();
  }
}
function gameFunctionality() {
  //Let's pizza intially fall if mouse is pressed
  if (mouseIsPressed) {
    pizzaIsFalling = true;
    showClickToStart = false;
    startTimer = true;
  }

  //Checks if pizzaIsFalling is true. If so, moves the pizza down.
  if (pizzaIsFalling) {
    drawPizza(pizzaX, pizzaY, 1);
    pizzaY += speedY + pizzasEaten / 10;
  }

  //Checks if the player lost
  if (pizzaY >= 350 || pizzaTime == 0) {
    gameOver = true;
  }

  //If game is lost and player clicks it will reset
  if (gameOver && mouseIsPressed) {
    gameOver = false;
    pizzaTime = 180;
    pizzaY = 0;
    pizzasEaten = 0;
  }
}
function timer() {
  //Timer for Game
  if (pizzaTime > 0 && startTimer) {
    pizzaTime--;
  }
}
function displayCounters() {
  //Time Display
  push();
  textSize(40);
  fill("white");
  if (startTimer) {
    text(pizzaTime, 50, 50);
  } else {
    text("180", 50, 50);
  }

  //Pizzas Eaten Display
  text(pizzasEaten, 350, 50);
  pop();
}
